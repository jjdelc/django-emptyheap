# -*- coding: utf 8 -*-

from django.utils.translation import ugettext as _

QUESTION = _(u'Question')
ANSWER = _(u'Answer')
UP_VOTE = _(u'Up vote')
UP_VOTE_PLURAL = _(u'Up votes')
DOWN_VOTE = _(u'Down vote')
DOWN_VOTE_PLURAL = _(u'Down votes')
VOTE_RESULT = _(u'Votes result')
BODY = _(u'Body')
COMMENT = _(u'Comment')
COMMENT_PLURAL = _(u'Comments')
ANSWER_COUNT = _(u'Number of answers')
QUESTION_CREATED = _(u'Your question was created')
ANSWER_SAVED = _(u'Your answer was saved')
YOU_HAVE_TO_LOGIN = _(u'You have to be logged in to do that.')
VOTE_CASTED = _(u'Your vote was saved')
INVALID_VOTE = _(u'Invalid vote value')
BEST_ANSWER = _(u'Looking for a best answer')
CONVERSATION = _(u'This is a conversation thread')
